function [ Der_dTdsd, dTds, dVds, Der_dTdthetad, dTdtheta, dVdtheta ] = Check_KinPotE( params, s, theta, sd, thetad, sdd, thetadd )
...
Der_dTdsd = (M1 + M2 +  I_B/r_B^2 + I_P2/r_P2^2)*sdd ...
    + 0.5*(I_B/r_B^2 + I_P1/r_P1^2) * Der_dlr1d2dsd;

%Derivative of T wrt s
dTds = (M1*(s+params.l5) + 2*M2*(s+params.l5 - ...
    0.5*params.l4))* thetad^2 + 0.5*( I_B/r_B^2 + I_P1/r_P1^2) * dlr1d2ds;
...
Der_dTdthetad = (M1*(s+params.l5)^2 + M2(0.25*r_M2^2 + 1/12*params.l4^2 + ...
    ...
    + 0.5*( I_B/r_B^2 + I_P1/r_P1^2 ) * Der_dlr1d2dthetad;

%Derivative of T wrt theta
dTdtheta = 0.5*(I_B/r_B^2 + I_P1/r_P1^2) * dlr1d2dtheta;
